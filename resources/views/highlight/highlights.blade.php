    ...
    <div class="uk-padding uk-light highlights highlights-bg" id="app">
        ...
        <div class="uk-grid">
                        ...
                        @foreach($top_posts_per_player[$top_player->name] as $post)
                            <div class="uk-flex uk-margin-small uk-margin-left">
                                ...
                                <p class="uk-padding-remove uk-margin-remove highlights-maptitle-container">
                                    <span class="highlights-maptitle">{{ $post->map_artist }} -
                                        {{ $post->map_title }}</span>&nbsp;<span
                                        class="highlights-mapdiff">[{{ $post->map_diff}}]</span>
                                </p>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="uk-width-1-3">
                ...
                <div class="uk-card uk-card-body uk-card-secondary uk-margin ">
                    ...
                    <bar-chart posts="{{ $score_per_day }}" name="scoreHistory" value-index="score_daily" height="170"
        ...
        <div class="uk-card uk-card-body uk-card-secondary uk-margin">
            @foreach($top_posts as $post)
                <p class="uk-margin-small"><span class="uk-badge uk-margin-small-right">{{$post->score}}</span>
                    {{ $post->player_name }} | {{ $post->map_artist }} - {{ $post->map_title }} [{{ $post->map_diff}}]</p>
            @endforeach
        </div>
